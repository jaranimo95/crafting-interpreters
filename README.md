...
4. Nested comments are already supported as part of block comments, as any nested comments can be considered part of the parent block comment. If nested comments should be detected whenever a block opener is encountered, then we'd need to maintain a stack to keep track of the possibly infinite recursive instances. This would immediately turn our lexical grammar from regular to context-free. Another option to maintain the regular grammar is to place a hard cap on the level of potential nesting.
